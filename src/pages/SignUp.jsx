...
const SignUp = () => {
  ...
  //   return fetch("https://passion-loop-server.vercel.app/users", {
  //     method: "POST",
  ...
  const saveUserToDB = (userProfile) => {
    ...
    return fetch("https://passion-loop-server.vercel.app/users", {
      ...
      .then((res) => res.json())
      .then((data) => {
        // ✅ Check for insertedId OR acknowledged
        if (data.insertedId || data.acknowledged) {
          ...
          // Since Firebase already logged them in, go to home or dashboard
          navigate("/");
        }
      })
      .catch((dbError) => {
        Swal.fire({
          ...
          text: "Firebase account created, but couldn't sync to database.",
        });
      });
  };
  ...
  const handleSignUp = (e) => {
    ...
    if (!imgRegExp.test(photo)) {
      toast.error(
        "Please enter a valid image URL (jpeg, png, gif, webp, svg).",
      );
      return;
    }
    ...
    <div className="min-h-screen bg-base-200 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm bg-base-100 rounded-2xl shadow-xl border border-base-300 p-6">
        ...
        <p className="text-center text-xs mt-4 text-base-content/60">
          ...
          <Link
            to="/auth/signin"
            className="text-[15px] font-semibold hover:underline text-blue-600"
          >
            Login
          </Link>
        </p>
